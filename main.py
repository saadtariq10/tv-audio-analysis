...
def process_audio(path):
    ...
    for chunk in chunks:
        if chunk.endswith(".wav"):
            ...
            # Delete the chunk after processing to free up space
            os.remove(f'audio_chunks/{chunk}')
    
    # Cleanup the audio_chunks directory after processing all chunks
...
def clean_temp_folder(folder):
    """
    Clean up the temporary folder by deleting all files.
    """
